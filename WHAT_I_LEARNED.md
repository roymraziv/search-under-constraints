...
- Whenever a query's operator class had a supporting index, the planner chose Bitmap Heap Scan every time — even for low-selectivity terms like "organic" that match ~3.5% of rows (34,730 actual rows / 1,000,000 total). The planner never fell back to a sequential scan when an operator-class-compatible index existed.
...
- Planner row estimates varied significantly. V0 Q3 estimated 70,667 rows versus 34,730 actual (70,667 / 34,730 = 2.0x overestimate). V1 Q3 estimated 10,096 versus 34,730 actual (34,730 / 10,096 = 3.4x underestimate). V3 Q3, with the trigram index providing better statistics, estimated 40,405 versus 34,730 actual (40,405 / 34,730 = 1.16x, much closer). Index statistics meaningfully improved planner accuracy.
...
- Indexed queries with high selectivity (V2 Q1, V4 Q8) achieved near-100% shared buffer hit ratios with essentially zero shared_read. Unindexed queries on the 1M table showed hit ratios around 10–12% (e.g. V0 Q4: 32,421 shared_hit / (32,421 + 268,451 total) = 10.8%), with ~268k shared_read pages per execution — nearly the entire table read from disk on each run.

### Key Deviations from Expectations
- Q7 (FTS for "organic") on V4 scaled far worse than expected. The GIN index provided a 13.2x speedup at 100k rows but only 1.9x at 1M rows. The planner still chose the index at 1M, but the speedup collapsed because the ~3.5% row selectivity (34,730 / 1,000,000) still required visiting those matching rows via random heap I/O, and at 1M rows that working set exceeded shared buffer capacity.
...
- Q7's scaling degradation comes down to cache capacity. At 100k rows, the GIN index on search_vector returns ~3,500 matching tids (3.5% of 100k), and the corresponding heap pages fit comfortably in shared buffers — most reads are cache hits. At 1M rows, the same 3.5% row selectivity means ~35,000 matches (3.5% of 1M). The V4 Q7 execution shows ~31,500 pages accessed (58 shared_hit + 31,451 shared_read from the benchmark data), nearly all from disk. That working set exceeds cache, so the indexed plan pays for disk reads that a sequential scan would avoid entirely via prefetching and OS readahead.
- Q3 on V3 follows the same mechanism. The trigram index returns a bitmap for the ~34,730 matching rows (3.5% row selectivity). At smaller table sizes, the bitmap is exact and the heap pages are cached. At 1M rows, the bitmap may become lossy (falling back to page-level granularity and forcing recheck), the heap working set spills out of cache, and per-match cost increases. Sequential scan avoids all of this — its cost grows linearly with table size because the I/O pattern is sequential and predictable.
